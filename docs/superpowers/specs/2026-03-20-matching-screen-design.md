...
1. `requireAuth`（STAFF以上）
2. OPEN案件 + AVAILABLE人材を全件取得
3. 既存マッチングの `[caseId, talentId]` セットを取得
4. 未マッチング組み合わせに対して `evaluateMatching` を呼び出し
5. `prisma.matching.upsert` で保存（`@@unique([caseId, talentId])` 制約のため）
6. 生成件数・スキップ件数を返す
...
- 既存マッチングは score 等を upsert で更新する（再生成で精度向上）。
...
- デフォルトで全チェック済み
- 「送信（コピー）」ボタン → チェック済みを順次 `SENT` に更新、本文を連結クリップボードコピー
