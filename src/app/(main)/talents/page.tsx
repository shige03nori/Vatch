      ...
      <main className="flex-1 overflow-y-auto">
        <div className="p-6">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            ...
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as TalentStatus | 'all')} className="bg-vatch-surface border border-vatch-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#38bdf8] transition-colors cursor-pointer">
              <option value="all">全ステータス</option>
              {(Object.keys(STATUS_CONFIG) as TalentStatus[]).map((s) => (<option key={s} value={s}>{STATUS_CONFIG[s].label}</option>))}
            </select>
            <select value={workStyleFilter} onChange={(e) => setWorkStyleFilter(e.target.value as WorkStyle | 'all')} className="bg-vatch-surface border border-vatch-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#38bdf8] transition-colors cursor-pointer">
            ...
            <select value={skillFilter} onChange={(e) => setSkillFilter(e.target.value)} className="bg-vatch-surface border border-vatch-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#38bdf8] transition-colors cursor-pointer">
          ...
          <div className="bg-vatch-surface border border-vatch-border rounded-lg overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {loading ? (
                    <tr><td colSpan={8} className="text-center py-12 text-vatch-muted">読み込み中...</td></tr>
                  ) : filtered.length === 0 ? (
                    <tr><td colSpan={8} className="text-center py-12 text-vatch-muted">該当する人材が見つかりません</td></tr>
                  ) : (
                    filtered.map((item, index) => {
                      ...
                      return (
                        <tr key={item.id} className={`border-b border-vatch-border/50 hover:bg-white/[0.02] transition-colors ${index === filtered.length - 1 ? 'border-b-0' : ''}`}>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex flex-wrap gap-1 max-w-[200px]">
                              {item.skills.slice(0, 3).map((skill) => (<span key={skill} className="px-1.5 py-0.5 bg-vatch-border/50 text-vatch-muted rounded text-xs">{skill}</span>))}
                          ...
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${workCfg.color} ${workCfg.bg}`}>{workCfg.label}</span>
                          </td>
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${statusCfg.color} ${statusCfg.bg}`}>{statusCfg.label}</span>
                          </td>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-center gap-1.5">
                              <button
                                ...
                                onClick={() => { setRowDeleteId(null); setSelectedTalent(item); }}
                              >
                                詳細
                              </button>
                              {isAdmin && (
                                isConfirmingDelete ? (
                                  <>
                                    <button onClick={() => handleDelete(item.id)} className="px-3 py-1 text-xs bg-red-500/20 border border-red-500 text-red-400 rounded transition-colors hover:bg-red-500/30">確認</button>
        ...
        <Modal open={selectedTalent !== null} onClose={closeModal}>
          <div className="bg-vatch-surface border border-vatch-border rounded-xl shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-vatch-border">
              <h2 id="modal-title" className="text-base font-bold text-white truncate pr-4">{selectedTalent?.name}</h2>
              <div className="flex items-center gap-2 shrink-0">
                {editing ? (
                  <>
                    <button onClick={() => { setEditing(false); setSaveError(null); setEditForm(toEditTalentForm(selectedTalent!)); }} className="px-3 py-1.5 text-xs border border-vatch-border text-vatch-muted rounded-lg hover:border-[#38bdf8] hover:text-[#38bdf8] transition-colors">キャンセル</button>
                    <button onClick={handleSave} disabled={saving} className="px-3 py-1.5 text-xs bg-[#38bdf8] text-black font-semibold rounded-lg disabled:opacity-50">{saving ? '保存中...' : '保存'}</button>
                  </>
                ) : (
                  <>
                    {isAdmin && (
                      deleteState === 'confirm' ? (
                        <>
                          <button onClick={() => handleDelete(selectedTalent!.id, true)} className="px-3 py-1.5 text-xs bg-red-500/20 border border-red-500 text-red-400 rounded-lg hover:bg-red-500/30 transition-colors">本当に削除</button>
                          <button onClick={() => setDeleteState('idle')} className="px-3 py-1.5 text-xs border border-vatch-border text-vatch-muted rounded-lg hover:text-white transition-colors">キャンセル</button>
                        </>
                      ) : (
                        <button onClick={() => setDeleteState('confirm')} className="px-3 py-1.5 text-xs border border-vatch-border text-vatch-muted rounded-lg hover:border-red-500 hover:text-red-400 transition-colors">削除</button>
                      )
                    )}
                    <button onClick={() => setEditing(true)} className="px-3 py-1.5 text-xs border border-vatch-border text-vatch-muted rounded-lg hover:border-[#38bdf8] hover:text-[#38bdf8] transition-colors">✏ 編集</button>
                  </>
                )}
                <button onClick={closeModal} className="text-vatch-muted hover:text-white transition-colors text-lg leading-none" aria-label="閉じる">✕</button>
              </div>
            </div>
              ...
              {editing ? (
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" value={editForm.name ?? ''} onChange={(e) => setEditForm((f) => ({ ...f, name: e.target.value }))} className="w-full bg-vatch-bg border border-vatch-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#38bdf8] transition-colors" />
                  </div>
                  <div>
                    <label className="block text-[10px] text-vatch-muted uppercase tracking-wide mb-1">居住地</label>
                    <input type="text" value={editForm.location ?? ''} onChange={(e) => setEditForm((f) => ({ ...f, location: e.target.value }))} className="w-full bg-vatch-bg border border-vatch-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#38bdf8] transition-colors" />
                  </div>
                  <div>
                    <label className="block text-[10px] text-vatch-muted uppercase tracking-wide mb-1">経験年数（年）</label>
                    <input type="number" min={0} value={editForm.experience ?? ''} onChange={(e) => setEditForm((f) => ({ ...f, experience: Number(e.target.value) }))} className="w-full bg-vatch-bg border border-vatch-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#38bdf8] transition-colors" />
                  </div>
                  <div>
                    <label className="block text-[10px] text-vatch-muted uppercase tracking-wide mb-1">希望単価（万円）</label>
                    <input type="number" min={1} value={editForm.desiredRate ?? ''} onChange={(e) => setEditForm((f) => ({ ...f, desiredRate: Number(e.target.value) }))} className="w-full bg-vatch-bg border border-vatch-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#38bdf8] transition-colors" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <select value={editForm.status ?? 'AVAILABLE'} onChange={(e) => setEditForm((f) => ({ ...f, status: e.target.value as TalentStatus }))} className="w-full bg-vatch-bg border border-vatch-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#38bdf8] transition-colors cursor-pointer">
                      {(Object.keys(STATUS_CONFIG) as TalentStatus[]).map((s) => (<option key={s} value={s}>{STATUS_CONFIG[s].label}</option>))}
                    </select>
                  </div>
                  <div className="col-span-2">
                    <label className="block text-[10px] text-vatch-muted uppercase tracking-wide mb-1">紹介元メール</label>
                    <input type="email" value={editForm.agencyEmail ?? ''} onChange={(e) => setEditForm((f) => ({ ...f, agencyEmail: e.target.value }))} className="w-full bg-vatch-bg border border-vatch-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#38bdf8] transition-colors" />
                  </div>
                ...
                selectedTalent && (
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${WORK_STYLE_CONFIG[selectedTalent.workStyle].color} ${WORK_STYLE_CONFIG[selectedTalent.workStyle].bg}`}>{WORK_STYLE_CONFIG[selectedTalent.workStyle].label}</span>
                    </div>
                    <div>
                      <div className="text-[10px] text-vatch-muted uppercase tracking-wide mb-1">ステータス</div>
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_CONFIG[selectedTalent.status].color} ${STATUS_CONFIG[selectedTalent.status].bg}`}>{STATUS_CONFIG[selectedTalent.status].label}</span>
                    </div>
                    <div>
                    ...
                    <div className="col-span-2">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {selectedTalent.skills.map((s) => (<span key={s} className="px-2 py-0.5 bg-vatch-border/50 text-vatch-muted rounded text-xs">{s}</span>))}
                      </div>
                    </div>
                    <div className="col-span-2">
                      <div className="text-[10px] text-vatch-muted uppercase tracking-wide mb-1">登録日時</div>
                      <div className="text-sm text-vatch-muted">{new Date(selectedTalent.createdAt).toLocaleDateString('ja-JP', { year: 'numeric', month: '2-digit', day: '2-digit' })}</div>
                    </div>
                  </div>
                )
              )}
