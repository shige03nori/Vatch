...
| 提案メール送信（NOW/SCHEDULED） | 🔶 部分実装 | ジョブ管理・暗号化・添付は完了。`sendEmail()` 呼び出しがコメントアウトのまま |
| 提案メール送信時の経歴書添付 | ✅ 完了 | `getLocalPath()` でパス解決・存在確認後に添付 |
| メール自動スケジュール送信（Cron） | 🔶 部分実装 | Cron ジョブ実装済み。実際の送信処理は上記と同様 |
...
| 課題 | 場所 | 優先度 |
|------|------|--------|
| `sendEmail()` 呼び出しのコメントアウトを外す | `src/app/api/proposals/[id]/send-email/route.ts` | 高 |
| `Talent.cv` テキストをメール生成プロンプトに渡す | 同上（`cv: ''` のまま） | 中 |
| 返信メールの自動解析 | 未実装 | 低 |
| AI精度フィードバック学習 | 未実装 | 低 |
| 売上予測・レポート | 未実装 | 低 |
